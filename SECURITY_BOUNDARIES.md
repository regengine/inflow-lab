...
- Cleartext `http` to a public host is refused: the API key rides in an `Authorization` header, so `http` would put it on the wire unencrypted. `REGENGINE_ALLOW_CLEARTEXT_DELIVERY=1` opts back in for a trusted network and relaxes the scheme only — private, loopback, and metadata hosts stay refused. `REGENGINE_ALLOW_PRIVATE_DELIVERY_HOSTS=1` already implies it, so the local-stack workflow is unaffected. Both must stay unset on shared or deployed profiles.
