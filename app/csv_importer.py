...
def _replay_window_warnings(
    ...
    own bundled demo fixtures and the suite's fixed-date canonical events;
    see MockRegEngineService.__init__ and issue #199. That default is
    exactly why this warning matters: in mock mode every stale row is
    accepted with a hash and a chain link, and without this the simulator
    gives no hint that an age limit even exists until the same batch is
    wholesale-rejected against live.
